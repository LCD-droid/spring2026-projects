...
Went through the steps to load the dataset, extract the activations, find direction, the PCA analysis, confounding analysis, plus length-adjusted results. Replicated the steering as well, though the steering results were evaluated via Pangram's api instead. 
...
One of the steering results was also leaking the prompt, which I didn't fully fix, so that's something I'll need to take care of next week. 
...
After projecting out length, accuracy at best layer (33): **85.5%**, also matching the findings of the original.
...
One interesting wrinkle: there's a consistent dip in classification accuracy around layers 8-10 before recovering sharply at layer 11. This shows up in both the accuracy plot and the PCA grid. I'm not sure what's happening computationally at those layers — worth investigating further.
...
The most interesting finding this week came from swapping the LLM judge for Pangram as the steering evaluator. The original study used GPT-4 to score "AI-likeness" on a 1-7 scale and found only a modest ~1 point shift from steering. Pangram tells a very different story (see also the graph below):
...
At multiplier -33, Pangram assigns only **2% probability** of AI authorship, meaning it was effectively fooled completely. The relationship is pretty monotonic across all five multipliers.
...
The steering examples also show a qualitative shift in the right direction just from reading it in (clipped) climate example above. 
